...
export default function TransfersContent({
  ...
  initialExecutions: TransferExecution[];
  initialAutoTopup: AutoTopupConfig | null;
}) {
  const { hideValues, setHideValues } = useView();
  const [accounts, setAccounts] = useState<Account[]>(initialAccounts);
  const [transfers, setTransfers] = useState<Transfer[]>(initialTransfers);
  const [executions, setExecutions] = useState<TransferExecution[]>(initialExecutions);
  const [autoTopup, setAutoTopup] = useState<AutoTopupConfig | null>(initialAutoTopup);
  ...
  useEffect(() => {
    ...
    if (initialTransfers.length === 0) fetchTransfers();
    if (initialExecutions.length === 0) fetchExecutions();
    if (!initialAutoTopup) fetchAutoTopupConfig();
  }, []);
