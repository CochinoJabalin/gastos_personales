...
export default function TransactionsContent({
  ...
  initialGroups: { group: string; type: string; count: number }[];
}) {
  ...
  const [groups, setGroups] = useState<{ group: string; type: string; count: number }[]>(initialGroups);
  ...
  useEffect(() => {
    if (initialBanks.length === 0) fetchBanks();
    if (initialYears.length === 0) fetchYears();
    ...
    if (initialGroups.length === 0) fetchGroups();
  }, []);
